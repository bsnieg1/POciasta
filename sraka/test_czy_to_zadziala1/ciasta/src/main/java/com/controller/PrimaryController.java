...
public class PrimaryController {
    ...
    private void addFiveMinutes() {
        totalSeconds += 300;
        updateTimerLabel();
        removeButton.setDisable(totalSeconds < 300);
    }

    @FXML
    private void removeFiveMinutes() {
        if (totalSeconds >= 300) {
            totalSeconds -= 300;
            updateTimerLabel();
        }
    ...
    private void openTimerDialog() {
        ...
        ButtonType stopButtonType = new ButtonType("Stop", ButtonBar.ButtonData.CANCEL_CLOSE);
        ButtonType addButtonType = new ButtonType("Add 5 mins", ButtonBar.ButtonData.OTHER);
        ButtonType removeButtonType = new ButtonType("Remove 5 mins", ButtonBar.ButtonData.OTHER);

        dialog.getDialogPane().getButtonTypes().addAll(startButtonType, stopButtonType, addButtonType, removeButtonType, ButtonType.CLOSE);
        ...
        dialog.setResultConverter(dialogButton -> {
            if (dialogButton == startButtonType) {
                timeline.play();
            } else if (dialogButton == stopButtonType) {
                timeline.pause();
